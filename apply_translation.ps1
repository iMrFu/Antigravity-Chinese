...
$dest = Join-Path $env:LOCALAPPDATA "Programs\antigravity\resources\app.asar"
$appPath = Join-Path $env:LOCALAPPDATA "Programs\antigravity\Antigravity.exe"
...
try {
    ...
    # 同时拷贝热更新规则文件 dynamic_rules.json
    $dynSrc = Join-Path $ScriptDir "dynamic_rules.json"
    $dynDest = Join-Path $env:LOCALAPPDATA "Programs\antigravity\dynamic_rules.json"
    if (Test-Path $dynSrc) {
        Copy-Item -Path $dynSrc -Destination $dynDest -Force
        Write-Host "   已同步热更新词典 dynamic_rules.json" -ForegroundColor Gray
    }
